...
## [v1.4.2-release-neoforge](http://github.com/Nova-Committee/Re-Avaritia/compare/v1.4.1-release-neoforge...v1.4.2-release-neoforge)
...
* Rebuild the Neutron Ring GUI with vanilla panels, a left library, and a 3D preview using actual Minecraft block models, fluids, and block-entity rendering. Left-drag rotates, right-drag pans, and the wheel zooms; left-click previews only. Selection, deselection, rename, delete, search, and preview reset use right-click menus.
* Use vanilla-styled Infinity Ring creation and management panels. World/access options and delete stay as panel buttons. The player roster is the only list, with a search field aligned to it; unmatched search names offer an add-friend row. Ban, visit, and role edits stay on the player-row context menu.
* Infinity Bucket management on Shift + right-click uses vanilla panels, fluid/creature lists, and a persistent search field aligned with the creature tab, with no transfer slot or redundant search menu action. Pick up a portable fluid container and right-click the fluid list to deposit, or a fluid row to extract. Pick up a mob bucket and right-click the list to store the creature, or an empty bucket and right-click a creature to extract it. Confirmed delete/clear remain right-click actions. In fluid mode, right-click a source to collect or a non-source block to output the selected fluid. With a creature selected, right-click water to release it without collecting the water. Air misses do nothing.
...
* Prioritize the selected Infinity Bucket creature over fluid pickup, so release into source water succeeds and a blocked release leaves both the water and stored contents unchanged.
* Fill an empty bucket from a stored Infinity Bucket creature, restoring bucket entity data and consuming one bucket of the matching fluid, or reject the transfer unchanged when the fluid is missing.
...
* Replace vanilla block drops when blaze-tool auto-smelting is enabled instead of spawning an additional set of drops. Convert the already-enchanted loot quantity by the smelting recipe ratio, without random skips or a second Fortune roll.
* Prevent a falling Extreme Anvil from dropping twice when it cannot be placed on landing.
* Only replace a Tesseract fluid bucket after its entire contents are accepted. Preserve rejected and empty buckets with their components, and prevent Shift + right-click from also depositing the original full bucket as an item.
* Select the Infinity Trident throwing pose before applying item transforms, and correct left-hand charging offsets in first- and third-person views.
