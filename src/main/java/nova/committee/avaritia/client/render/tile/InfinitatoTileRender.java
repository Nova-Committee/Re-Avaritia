...
import nova.committee.avaritia.init.mixin.client.AccessorModelManager;
...
public class InfinitatoTileRender implements BlockEntityRenderer<InfinitatoTile> {
    ...
    public static final String DEFAULT = "infinitato";
    ...
    private static ResourceLocation taterLocation(String name) {
        return new ResourceLocation(Static.MOD_ID, "compact" + "/" + normalizeName(name));
    }
    ...
    public void render(@Nonnull InfinitatoTile potato, float partialTicks, PoseStack ms, @Nonnull MultiBufferSource buffers, int light, int overlay) {
        ...
        ms.pushPose();
        //IClientXplatAbstractions.INSTANCE.fireRenderTinyPotato(potato, potato.name, partialTicks, ms, buffers, light, overlay);//event
        ms.popPose();
