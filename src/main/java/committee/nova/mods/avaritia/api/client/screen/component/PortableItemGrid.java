...
public final class PortableItemGrid extends AbstractWidget {
    ...
    public void onClick(MouseButtonEvent event, boolean doubleClick) {
        draggingScrollbar = maxScroll() > 0 && PortableLayout.contains(scrollbar, event.x(), event.y());
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (!visible || !active || !isMouseOver(mouseX, mouseY) || scrollY == 0) {
            return false;
        }
    ...
    public void renderTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (index >= 0 && !draggingScrollbar) {
            ItemStack stack = items.get(index);
            graphics.setTooltipForNextFrame(minecraft.font, tooltipProvider.apply(stack), stack.getTooltipImage(), stack, mouseX, mouseY);
        }
    }
