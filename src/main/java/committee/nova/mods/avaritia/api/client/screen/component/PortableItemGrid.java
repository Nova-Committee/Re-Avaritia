...
public final class PortableItemGrid extends AbstractWidget {
    ...
    public PortableItemGrid(Minecraft minecraft, int x, int y, int columns, int rows,
        ...
        this.tooltipProvider = stack -> Screen.getTooltipFromItem(minecraft, stack);
        updateGeometry();
    }
    ...
    private int indexAt(double mouseX, double mouseY) {
        if (!visible || !active || !PortableLayout.contains(content, mouseX, mouseY)) return -1;
        double x = mouseX - content.left() - 2;
        double y = mouseY - content.top() - 2;
        if (x < 0 || y < 0) return -1;
        int column = (int) (x / STEP);
        int row = (int) (y / STEP);
        if (column >= columns || row >= rows || x % STEP >= 18 || y % STEP >= 18) return -1;
    ...
    protected void onDrag(double mouseX, double mouseY, double dragX, double dragY) {
        if (draggingScrollbar) scrollFromMouse(mouseY);
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (!visible || !active || !isMouseOver(mouseX, mouseY) || scrollY == 0) return false;
        setScrollOffset(scrollOffset - (int) Math.signum(scrollY));
        return true;
    }
    ...
    protected void renderWidget(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int cell = 0; cell < columns * rows; cell++) {
            ...
            if (index >= items.size() || items.get(index).isEmpty()) continue;
