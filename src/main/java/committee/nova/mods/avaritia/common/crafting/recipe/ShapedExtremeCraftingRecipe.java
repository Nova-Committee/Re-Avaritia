...
public class ShapedExtremeCraftingRecipe implements ISpecialRecipe{
    ...
    @Override
    public @NotNull NonNullList<ItemStack> getRemainingItems(@NotNull ItemStackHandler inventory) {
        if (this.transformers != null) {
            var remaining = NonNullList.withSize(inventory.getSlotCount(), ItemStack.EMPTY);
            int size = (int) Math.sqrt(inventory.getSlotCount());
    ...
    private boolean checkMatch(ItemStackHandler inventory, int x, int y, boolean mirror) {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                int k = i - x;
                int l = j - y;
