...
public class InfinityBucketMenu extends BaseMenu {
    ...
    public InfinityBucketMenu(int id, Inventory playerInventory, FriendlyByteBuf buf) {
        this(id, playerInventory, -1);
    }

    public InfinityBucketMenu(int id, Inventory playerInventory) {
        this(id, playerInventory, -1);
    }
    ...
    public InfinityBucketMenu(int id, Inventory playerInventory, int bucketSlot) {
        ...
        addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return InfinityBucketMenu.this.bucketSlot;
            }

            @Override
            public void set(int value) {
                InfinityBucketMenu.this.bucketSlot = value;
            }
        });
    }
    ...
    private static Slot lockedOffhandSyncSlot(Inventory inventory) {
        return new Slot(inventory, OFFHAND_INVENTORY_SLOT, 0, 0) {
            @Override
            public boolean mayPickup(@NotNull Player player) {
                return false;
            }
    ...
    public boolean ownerValid() {
        ...
        return (player.level().isClientSide() || current == owningBucket)
                && !current.isEmpty()
    ...
    private boolean transferCarried(ItemStack bucket, int fluidIndex) {
        ...
        IFluidHandlerItem itemHandler = FluidUtil.getFluidHandler(one).resolve().orElse(null);
        IFluidHandlerItem bucketHandler = FluidUtil.getFluidHandler(bucket).resolve().orElse(null);
