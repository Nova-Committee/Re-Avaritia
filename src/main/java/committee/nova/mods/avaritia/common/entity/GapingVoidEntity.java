...
public class GapingVoidEntity extends Entity {
    ...
    public GapingVoidEntity(EntityType<?> p_19870_, Level p_19871_) {
        ...
        if (level() instanceof ServerLevel) {
            fakePlayer = FakePlayer.get((ServerLevel) level(), Static.avaritiaFakePlayer);
        }
    }
            ...
            nommed.stream()
                    ...
                    .forEach(entity -> {
                        if (entity instanceof EnderDragon dragon) {
                            dragon.hurt(dragon.head, ModDamageTypes.infinity(level(), user), 1000.0f);
                            dragon.setHealth(0);
                        } else if (entity instanceof WitherBoss wither) {
                            wither.setInvulnerableTicks(0);
                            wither.hurt(ModDamageTypes.infinity(level(), user), 1000.0f);
                        } else entity.hurt(ModDamageTypes.infinity(level(), user), 1000.0f);
                    });
                ...
                for (int z = -blockrange; z <= blockrange; z++) {
                    for (int x = -blockrange; x <= blockrange; x++) {
                        ...
                        if (dist <= nomrange && !level().getBlockState(blockPos).isAir()) {
                            ...
                            BlockEvents.BreakEvent event = new BlockEvents.BreakEvent(level(), blockPos, state, fakePlayer);
                            //MinecraftForge.EVENT_BUS.post(event);
                            if (!event.isCanceled()) {
                                float resist = state.getBlock().getExplosionResistance();
                                if (resist <= 10.0) {
                                    state.getBlock().dropFromExplosion(new Explosion(level(), null, blockPos.getX(),
