...
public class InfinityChestRender implements BlockEntityRenderer<InfinityChestTile> {
    ...
    @Override
    public void render(InfinityChestTile pBlockEntity, float pPartialTick, @NotNull PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        Level level = pBlockEntity.getLevel();
        boolean flag = level != null;
        BlockState blockstate = flag ? pBlockEntity.getBlockState() : ModBlocks.infinity_chest.get().defaultBlockState().setValue(ChestBlock.FACING, Direction.SOUTH);
            pPoseStack.pushPose();
            ...
            f1 = 1.0F - f1;
            f1 = 1.0F - f1 * f1 * f1;
            VertexConsumer vertexconsumer = new Material(Sheets.CHEST_SHEET, Const.rl("block/chest/infinity")).buffer(pBuffer, RenderType::entityCutout);
            this.render(pPoseStack, vertexconsumer, this.lid, this.lock, this.bottom, f1, pPackedLight, pPackedOverlay);
