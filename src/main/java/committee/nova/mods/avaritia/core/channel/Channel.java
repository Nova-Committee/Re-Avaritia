...
public abstract class Channel implements IItemHandler, IFluidHandler, IEnergyStorage {
    ...
    public int canStorageAmount(ItemStack itemStack) {
        if (!itemStack.isComponentsPatchEmpty()) return 0;
    ...
    public int canStorageAmount(FluidStack fluidStack) {
        if (!fluidStack.isComponentsPatchEmpty()) return 0;
    ...
    /**
     * @param itemStack 浼氳淇敼锛屽涓嶈繘鍘讳細鏈変綑锛?
     * @return 瀛樿繘鍘荤殑閲?
     */
    public int addItem(ItemStack itemStack) {
        if (!itemStack.isComponentsPatchEmpty() || itemStack.isEmpty()) return 0;
    ...
    /**
     * @param fluidStack 浼氳淇敼锛屽涓嶈繘鍘讳細鏈変綑锛?
     * @return 瀛樿繘鍘荤殑閲?
     */
    public int addFluid(FluidStack fluidStack) {
        if (!fluidStack.isComponentsPatchEmpty() || fluidStack.isEmpty()) return 0;
     ...
     * 濉厖鐗╁搧鍙犲爢锛屼笉闄愬埗鏁伴噺銆?
     *
     * @param itemStack 瑕佸～鍏呯殑鐗╁搧
     * @param count     瑕佸～鍏呯殑鏁伴噺锛岃礋鏁颁负鎵ｉ櫎銆?
     */
    public void fillItemStack(ItemStack itemStack, int count) {
        if (itemStack.isEmpty() || count == 0 || !itemStack.isComponentsPatchEmpty()) return;
        String itemId = StorageUtils.getItemId(itemStack.getItem());
        if (storageItems.containsKey(itemId)) {
            long storageCount = storageItems.get(itemId);
            long remainingSpaces = Long.MAX_VALUE - storageCount;
    ...
    public void fillFluidStack(FluidStack fluidStack, int count) {
        if (fluidStack.isEmpty() || count == 0 || !fluidStack.isComponentsPatchEmpty()) return;
        String fluidId = StorageUtils.getFluidId(fluidStack.getFluid());
        if (storageFluids.containsKey(fluidId)) {
    ...
    public void removeItem(String itemId, long count) {
        if (count <= 0 || !storageItems.containsKey(itemId)) return;
        long storageCount = storageItems.get(itemId);
        if (count < storageCount) {
            storageItems.replace(itemId, storageCount - count);
            onItemChanged(itemId, false);
    ...
    public void removeEnergy(Long amount) {
        if (amount == null || amount <= 0 || !storageEnergies.containsKey("avaritia:forge_energy")) return;
    ...
    public void removeEnergy(String energyId, Long amount) {
        if (amount == null || amount <= 0 || !storageEnergies.containsKey(energyId)) return;
    ...
    public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
        if (stack.isEmpty() || !stack.isComponentsPatchEmpty()) return stack;
    ...
    public int fill(FluidStack resource, FluidAction action) {
        if (resource.isEmpty() || !resource.isComponentsPatchEmpty()) return 0;
    ...
    public @NotNull FluidStack drain(FluidStack resource, FluidAction action) {
        ...
        return resource.copyWithAmount(count);
    }
    ...
    public int extractEnergy(int maxExtract, boolean simulate) {
        if (maxExtract <= 0 || !storageEnergies.containsKey("avaritia:forge_energy")) return 0;
        long storageCount = storageEnergies.get("avaritia:forge_energy");
