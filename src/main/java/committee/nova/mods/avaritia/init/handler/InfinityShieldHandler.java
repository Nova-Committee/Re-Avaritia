...
public final class InfinityShieldHandler {
    ...
    public static void onDefendingShieldBlock(LivingShieldBlockEvent event) {
        ...
        // Preserve the piercing-arrow and BYPASSES_SHIELD exceptions from 26.1.2.
        DamageSource source = event.getDamageSource();
        boolean piercing = source.getDirectEntity() instanceof AbstractArrow arrow && arrow.getPierceLevel() > 0;
        if (player.isUsingItem()) {
            ItemStack useItem = player.getUseItem();
            if (useItem.is(ModItems.infinity_shield.get())
                    && !source.is(DamageTypeTags.BYPASSES_SHIELD) && !piercing) {
        ...
        serverLevel.getServer().execute(() -> {
            if (target.isAlive()) {
                target.hurt(ModDamageTypes.causeRandomDamage(player), blocked);
            }
        });
    }
    ...
    public static void onDefendingProjectile(ProjectileImpactEvent event) {
        ...
        projectile.setDeltaMovement(projectile.getDeltaMovement().reverse().scale(0.9D));
        event.setCanceled(true);
    }
